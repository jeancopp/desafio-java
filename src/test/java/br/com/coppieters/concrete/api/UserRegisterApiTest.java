...
public class UserRegisterApiTest {
    ...
    public void withRightData_OnRecord_ShouldRecord() throws UserRecordedPreviouslyException {
        ...
        assertEquals(HttpStatus.OK, returnValue.getStatusCode());
        ...
        assertEquals(LAST_LOGIN_DATE, body.getLast_login());
        assertEquals(MODIFIER_DATE, body.getModified() );
        assertEquals(CREATED_DATE, body.getCreated() );
        assertEquals(TOKEN, body.getToken());
        assertEquals(ID, body.getId().longValue());
    }
